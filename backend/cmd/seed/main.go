...
func main() {
	...
	err = tx.QueryRowContext(ctx, "SELECT id FROM locations WHERE code = 'MAIN'").Scan(&locationIDStr)
	...
	if err == sql.ErrNoRows {
		...
		log.Printf("Failed to query location: %v", err)
		return
	} else {
	...
	if err != nil {
		...
		tx.QueryRowContext(ctx, "SELECT id FROM customers WHERE account_number='ACME-001'").Scan(&customerID)
	}
	...
	if err != nil {
		// Might exist, query it
		tx.QueryRowContext(ctx, "SELECT id FROM drivers WHERE name='Colton (Dev)'").Scan(&driverID)
	}
	...
	routeID := uuid.New()
	_, err = tx.ExecContext(ctx, `
		INSERT INTO delivery_routes (id, driver_id, vehicle_id, scheduled_date, status, stop_count)
		VALUES ($1, $2, $3, CURRENT_DATE, 'SCHEDULED', 1)
